<x-app-layout>
    <x-slot name="header">
        <h2 class="h2 text-gradient">Create New Event</h2>
    </x-slot>
    ...
    <div class="glass-panel" style="max-width: 800px; margin: 0 auto;">
        <form action="{{ route('organizer.events.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-2 gap-6">
                ...
                <div class="form-group">
                    ...
                    <select name="is_public" class="form-control" required>
                        ...
                        <option value="0" {{ old('is_public') == '0' ? 'selected' : '' }}>Private (Hidden from attendees)</option>
                    </select>
                ...
                <div class="form-group col-span-2">
                    ...
                    <div id="drop-zone" style="border: 2px dashed var(--border); border-radius: var(--radius-md); padding: 2rem; text-align: center; cursor: pointer; background: rgba(0,0,0,0.02); transition: all 0.2s;">
                        ...
                        <img id="image-preview" style="display: none; max-height: 200px; margin: 0 auto; border-radius: 8px; box-shadow: var(--shadow-sm);" />
                    </div>
                    <input type="file" name="image" id="file-input" class="form-control" accept="image/*" style="display: none;">
                    <p class="text-muted text-xs mt-1">Max 2MB. JPEG, PNG, or GIF.</p>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="capacity" min="1" class="form-control" value="{{ old('capacity') }}" required>
            ...
            <div class="form-group mt-4">
                ...
                <textarea name="description" id="description" class="form-control" rows="5" required oninput="document.getElementById('char-count').innerText = this.value.length">{{ old('description') }}</textarea>
                <div class="text-xs text-muted mt-1 text-right"><span id="char-count">{{ strlen(old('description') ?? '') }}</span> characters</div>
    ...
    <script>
        // Set today as minimum for start_date
